...
\begin{mdframed}[style=DefinitionFrame]
    \begin{defn}
    \end{defn}
    \cusTi{Función periódica continua}
    \cusTe{$x: \setR \longrightarrow \setC$ es una función periódica si y solo si}
    \begin{equation*}
        \fx[x]{t} = \fx[x]{t + T_0}
    \end{equation*}
...
El período fundamental $T_0$ de $\fx[x]{t}$ va a ser el mínimo tiempo en que todas completen un ciclo entero:
\[
...
\begin{mdframed}[style=DefinitionFrame]
    ...
    \end{defn}
    \cusTi{Función periódica discreta}
...
Se utiliza comúnmente para modelar señales que \inQuotes{comienzan} en un instante específico y permanecen activas a partir de allí.

\begin{mdframed}[style=DefinitionFrame]
    \begin{defn}
    \end{defn}
    \cusTi{Escalón unitario}
    \[
        \fx[u]{t - t_0} =
        \left\{
        \begin{aligned}
...
Pero es posible dar una noción de derivada uniendo los dos tramos del escalón con una recta que va \inQuotes{desde la izquierda del cero hasta la derecha del cero} para obtener una función continua.
...
Personalmente, me gusta pensar que se trata de una función que tiene 2 dimensiones temporales: una en el sentido de $t$ y otra hacia \inQuotes{adentro}.
De manera que es posible que la función tenga un cambio \inQuotes{suave} y discontinuo simultáneamente para un único instante $t_0$.
Así, el reloj no estaría frenándose sino que estaría usando otras manecillas.

Vemos que cuando el \inQuotes{corte en cero} tiende a unirse, la recta que une ambos tramos es una línea vertical con pendiente infinita.
