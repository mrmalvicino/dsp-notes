...
El procedimiento general para determinar la función de transferencia consiste en aplicar al sistema una entrada de la forma \( \fx[x]{t} = e^{\iu \omega t} \), obtener la salida correspondiente \( \fx[y]{t} \), y luego calcular el cociente:
...
Este sistema simplemente multiplica la señal de entrada por una constante $C \in \setC$:
\[
    \fx[y]{t} = C \, \fx[x]{t}
\]

Si la entrada es $\fx[x]{t} = e^{\iu \omega t}$, la salida será $\fx[y]{t} = C \, e^{\iu \omega t}$:
\[
    \fx[H]{\iu \omega} = \frac{C \, e^{\iu \omega t}}{e^{\iu \omega t}} = C
\]

\subsection{Retardador}

Este sistema desplaza la señal en el tiempo una cantidad $t_0$ hacia el futuro:
\[
    \fx[y]{t} = \fx[x]{t - t_0}
\]

Si la entrada es $\fx[x]{t} = e^{\iu \omega t}$, la salida será:
\begin{align*}
    \fx[y]{t}
    & = e^{\iu \, \omega \, (t - t_0)}
    \\
...
Este sistema devuelve la derivada de la señal de entrada:
\[
    \fx[y]{t} = \frac{\dif}{\dif t} \fx[x]{t}
\]

Para $\fx[x]{t} = e^{\iu \omega t}$, la derivada es:
\[
...
Este sistema devuelve la integral de la señal de entrada:
\[
    \fx[y]{t} = \int_{-\infty}^{t} \fx[x]{\tau} \, \dif \tau
\]

Si $\fx[x]{t} = e^{\iu \omega t}$, entonces:
\begin{align*}
    \fx[y]{t}
    & = \int_{-\infty}^{t} e^{\iu \omega \tau} \, \dif \tau
    \\
    & = \frac{1}{\iu \omega} \, e^{\iu \omega t} + C
\end{align*}

Ignorando la constante de integración si el sistema es de tipo \inQuotes{respuesta al impulso cero}, se tiene:
\[
